...
def get_cifar10(root, n_labeled,
    ...
    test_dataset = torchvision.datasets.CIFAR10(root, train=False, download=download)
    train_labeled_idxs, train_unlabeled_idxs, val_idxs = train_val_split(test_dataset.targets, int(n_labeled/10)) # was base_dataset.target

    train_labeled_dataset = CIFAR10_labeledmod(root, train_labeled_idxs, train=False, transform=transform_train) # train=True
    train_unlabeled_dataset = CIFAR10_unlabeled(root, train_unlabeled_idxs, train=False, transform=TransformTwice(transform_train)) # train = True. possibly modify the transforms
    val_dataset = CIFAR10_labeled(root, val_idxs, train=True, transform=transform_val, download=True)  # possibly change train = False here and above
    test_dataset = CIFAR10_labeled(root, val_idxs, train=False, transform=transform_val, download=True) # might be good to use seperate indices than the ones in train_labeled i.e use val_idxs here too
